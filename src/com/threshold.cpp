...
{
    po::variables_map vm=parseArgs(argc, argv, {"inputImage", "outputImage", "thresholdLow", "thresholdHigh"});

     // custom loading function defined in common.cpp
    Mat image = imreadHelper(vm["inputImage"].as<std::string>());


    Mat res_image = threshold(image, vm["thresholdLow"].as<float>(), vm["thresholdHigh"].as<float>());
